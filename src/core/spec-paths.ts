...
**活动变更 ID**：\`.sparrow/active-change.json\` 的 \`changeId\`；若为空且 \`change/current/\` 仅有一个子目录，则使用该目录名。新建变更时须向用户确认 \`{change-id}\`：给出 1～3 个建议 kebab-case 名称，并允许自定义（详见 \`requirement/requirements.md\`「活动变更 ID 确认纪律」）。**确认前禁止**在 \`change/current/\` 下创建子目录；用户中止则 \`current/\` 保持为空。**首次 archive 前 master 保持为空目录**，requirement 产出只写在 \`change/current/{change-id}/\`。
