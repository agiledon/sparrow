...
test('generated sparrow-state.mjs is bundled from agent-scripts (single source)', () => {
  const mjsPath = join(
    process.cwd(),
    'src/schemas/templates/shared/scripts/sparrow-state.mjs'
  );
  const body = readFileSync(mjsPath, 'utf-8');
  assert.match(body, /AUTO-GENERATED by scripts\/generate-sparrow-state-mjs\.mjs/);
  assert.match(body, /checkArchiveReadiness|check-archive/);
...
test('ensure-change-workspace --create uses sparrow-state.mjs for change and pipeline', () => {
  ...
  copyFileSync(
    join(process.cwd(), 'src/schemas/templates/shared/scripts/sparrow-state.mjs'),
    join(scriptsDir, 'sparrow-state.mjs'),
  );
  copyFileSync(
    join(
      process.cwd(),
      'src/schemas/templates/skills/sparrow-requirement/scripts/ensure-change-workspace.mjs',
    ),
  ...
  assert.equal(state['active-change'].changeId, 'my-feature');
