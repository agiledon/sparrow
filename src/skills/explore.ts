...
3. **按维度有序推进**：优先覆盖核心流程和参与者，再深入业务规则和边界条件，最后覆盖异常场景和质量属性
4. **闭环确认**：当某一维度的所有问题都得到用户确认后，总结该维度的结论，再进入下一维度
5. **不过度提问**：如果用户的问题已经清晰覆盖某一维度，不要为了提问而提问
...
✅ 完成 sparrow-explore 后，请执行 **sparrow-arch**（产品级）—— 基于 prd-business.md 和 prd-quanlity.md，划分子领域并定义业务架构和应用架构。`;
