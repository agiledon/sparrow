...
1. **互动式交互**：遵守 \`common/always/interactive-interaction.md\` 与 \`requirement/requirements.md\`「Grill Me 提问纪律（阶段一）」
...
- **完整性自检与补漏**：识别完成后，按业务流程逐段核对，确认是否有遗漏的业务服务；若发现遗漏，通过 Grill Me 向客户确认是否补充（不得擅自添加未经确认的服务）
...
### 阶段三：UI 设计探索
- [ ] 用户旅程完整：每类用户从进入到完成目标的旅程无断点
- [ ] 无「孤儿」页面：每个页面都有进入来源与去向（返回 / 前进 / 跳转入口）
- [ ] 关键场景已覆盖（如创建后可返回列表、列表可进详情 / 编辑、多详情有切换）
- [ ] 页面内与页面间交互方式从用户体验角度设计（操作有反馈、无死胡同）
- [ ] 用户旅程与 UI 页面体现了前后衔接与交互关系
- [ ] 原型严格按 UI 页面设计要求给出，并体现完整导航
...
1. 遵守 \`common/always/interactive-interaction.md\` 与 \`requirement/requirements.md\`「Grill Me 提问纪律（阶段三）」；给出推荐答案，并在末尾明确提示确认选项
...
1. **旅程完整性**：从用户交互角度闭环核对——每类用户的每个关键触点都能前进到下一环节或返回上一环节，旅程无断点。
2. **页面完整性（禁止「孤儿」页面）**：每个页面都必须有进入来源与去向——创建产品后能返回产品列表页、列表页能进入详情 / 编辑页、多详情分页之间有切换操作等；禁止任何页面既无法回退、又无法前进、也无法通过导航 / 链接进入其他页面。
3. **页面内与页面间交互**：从用户体验角度设计交互方式——操作有明确反馈，跳转关系清晰，无死胡同。
4. **落地到产物**：上述前后衔接与交互须在用户旅程、UI 页面定义中明确写出，并严格按 UI 页面设计要求在原型中体现（每个原型页面提供返回 / 前进 / 跳转入口）。
