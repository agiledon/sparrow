...
4. 确认后创建 `docs/sparrow/change/current/{change-id}/`（及 `requirement/business/`、`requirement/quality/`、`requirement/ui/`、`architecture/`、`design/`），写入 `proposal.md`，并更新 `.sparrow/sparrow-state.json` 的 `active-change.changeId`。
...
写完 BS、进入 quality/UI 前，对每条 EBP：

1. **链完整**：步骤有序；相邻步骤交接清晰（角色/触发/前置结果）。
2. **无遗漏**：每个需目标系统处理的步骤都有对应 BS；缺口须 Grill Me 确认补识别或明确「非本系统」。
3. **无孤儿 BS**：每个 BS 至少出现在一条 EBP 中。
4. **catalog 落盘**：含 `EBP-*`、步骤序号、对应 `BS-*`、涉及 `S-*`/`SD-*`。

## 业务服务（MUST / MUST NOT）

1. **必须全面覆盖**沿 EBP 识别出的系统步骤；名称必须为动词短语（动宾）。
2. **必须包含**：服务编号（`BS-*`）、服务名、服务描述、触发事件、基本流程、替代流程、**EARS 验收标准**、追溯（SD/C/S/EBP）。
