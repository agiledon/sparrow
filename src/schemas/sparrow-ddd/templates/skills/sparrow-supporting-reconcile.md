...
4. **不触发 revise 流程**：不自动创建 \`change/current/{change-id}/\`、不自动触发 sparrow-archive。
...
| 层级 | 路径 | 前提 |
|------|------|------|
| 产品需求 | \`requirement/business/prd-business.md\` | 文件存在 |
| 质量属性 | \`requirement/quality/prd-quality.md\` | 文件存在 |
...
加载 \`docs/sparrow/change/current/{activeChangeId}/project.md\` 获取已有 slug 列表，**仅遍历已存在的 \`design/{slug}/\` 目录**。
...
3. 读取 \`docs/sparrow/change/current/{activeChangeId}/project.md\` 获取已有 slug 与文档索引
...
|----------|------------------|---------------------|
| 业务需求 | 业务服务、流程、验收标准、用户可见行为变化 | \`design/{slug}/spec.md\` + \`requirement/business/prd-business.md\` |
| 技术决策 / 质量属性 | API 契约、技术栈、架构模式、NFR（性能/安全/可用性） | \`design/{slug}/api.md\`、\`tech.md\` + \`requirement/quality/prd-quality.md\` |
...
若检测到活动 change-id（\`docs/sparrow/change/current/\` 有未归档变更），提示用户：**架构级变更应走 revise 流程；reconcile 仅对齐已有规格，不替代 revise**。
