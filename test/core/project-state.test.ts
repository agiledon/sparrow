...
test('generated sparrow-state.mjs is bundled from agent-scripts (single source)', () => {
  const mjsPath = join(
    process.cwd(),
    'src/content/workflows/_shared/scripts/sparrow-state.mjs'
  );
  const body = readFileSync(mjsPath, 'utf-8');
  assert.match(body, /AUTO-GENERATED by src\/kernel\/runtime\/agent-scripts\/bundle-sparrow-state\.mjs/);
...
test('ensure-change-workspace --create uses sparrow-state.mjs for change and pipeline', () => {
  ...
  copyFileSync(
    join(
      process.cwd(),
      'src/content/workflows/sparrow-requirement/scripts/ensure-change-workspace.mjs',
    ),
